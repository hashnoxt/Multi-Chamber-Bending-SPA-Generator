...
class Population(object):
    ...
    def __init__(self, *items, **kw):
        ...
        if kw.has_key('init'):
            self.initPopulation = kw['init']
        
        if not items:
            for i in xrange(self.initPopulation):
                self.add(self.species())
        else:
            self.add(*items)
            flag = self.initPopulation-len(items)
            if flag>0:
    ...
    def fitness(self):
        ...
        """
        self.fitnesses = [org.fitnessValue for org in self.organisms]
    ...
    def recordResult(self):
        ...
        raise Exception("Method 'recordResult' not implemented")