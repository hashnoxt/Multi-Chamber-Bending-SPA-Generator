...
import matplotlib
# Matplotlib uses the png renderer for the images. Needed for cluster.
matplotlib.use('Agg')
...
def run_abq(inp, outdir, np, descr):
    ...
    if not inp.endswith('.inp'):
        utility.print_error(
            'Input file must be a .inp Abaqus run file.', False)
        return 0
...
def run_post(inp, outdir, descr):
    ...
    try:
        ...
        return 0
    try:
        import create_result_plots
        import numpy
        from matplotlib import pyplot
        # Don't use italics for mathmode.
        pyplot.rc('mathtext', default='regular')
        fig, ax = pyplot.subplots()
        create_result_plots.plot_fem_data(ax, 'data.rpt')
        lgd = ax.legend(loc='best', frameon=False, framealpha=0)
        pyplot.savefig('plot.png', bbox_extra_artists=(
        ...
        if path.exists('data-energy.rpt'):
            ...
            lgd = ax.legend(loc='best', frameon=False, framealpha=0)
            pyplot.savefig('plot-energy.png',
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        'inp', nargs='+', help='Abaqus INP file or list of INP files.')
    parser.add_argument('-c', '--cluster', choices=['bellatrix', 'aries', 'deneb',
                        'deneb_himem'], help='Create and submit Slurm jobs on the cluster.')
    parser.add_argument('-a', '--all', action='store_true',
                        help='Run all the actions (eg, --run and --post).')
    parser.add_argument('-r', '--run', action='store_true',
                        help='Run an Abaqus simulation.')
    parser.add_argument('-p', '--post', action='store_true',
                        help='Postprocess results.')
    parser.add_argument('-nn', '--nodes', metavar='NN', default=1,
                        type=int, help='Number of nodes to use (default=1).')
    parser.add_argument('-np', '--procs', metavar='NP', type=int,
                        help='Number of processors to use per node (default=8 for non-cluster).')
    parser.add_argument('--sort', action='store_true',
                        help='Numerically sort inputs from smallest to largest.')
    parser.add_argument(
        '--descr', help='Optional description to add to directory name.', default='')
    parser.add_argument('--qtime', default='00:30:00', metavar='hh:mm:ss',
                        help='Requested queue time on cluster (default=30min).')
    ...
    if args.cluster:
        cp = cluster_properties(args.cluster)
    if not args.procs:
        if args.cluster:
            np = cp['np']
        else:
    ...
    for inp in args.inp:
        if args.cluster:
            r, jobfile = run_slurm(inp, cp, args.qtime,
                                   args.outdir, args.nodes, np, args.descr)
            tC += r
            tA += 1
            # Submit the job.
            if args.array < 0:
    ...
    if args.array >= 0:
        run_array(args.outdir, jobs, args.array, cp,
                  args.qtime, args.nodes, np, args.descr)
